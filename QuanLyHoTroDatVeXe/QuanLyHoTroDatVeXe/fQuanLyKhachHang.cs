...
namespace QuanLyHoTroDatVeXe
{
    public partial class fQuanLyKhachHang : Form
    {
        ...
        {
            dsKhachHang.DataSource = HanhKhachDAO.Instance.layDsKhachHang();
        ...
        void themDuLieu()
        {
            try
            {
                ...
                {
                    int sdtTim = HanhKhachDAO.Instance.timKH(sdt);
                    if (sdtTim == sdt)
                        MessageBox.Show("Lỗi!! Số điện thoại đã tồn tại", "Thêm khách hàng", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    else
                    {
                        bool ketQua = HanhKhachDAO.Instance.themKH(sdt, cmnd, ht, gt, dc, email);
                        if (ketQua)
                        {
        ...
        private void BtSua_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string dc = txtDiaChi.Text;
                string email = txtEmail.Text;
                bool ketQua = HanhKhachDAO.Instance.suaKHBangSDT(sdt, cmnd, ht, gt, dc, email);
                if (ketQua)
                {
        ...
        private void BtXoa_Click(object sender, EventArgs e)
        {
            ...
            {
                HanhKhachDAO.Instance.xoaKHBangSDT(sdt);
                hienThiDanhSach();
            }
        }
        #endregion
